...
class TabNetEncoder(nn.Module):
    def __init__(self, batch_size, input_dim, output_dim, step_num, Nd=64, Na=64, gamma=1):
        ...
        self.TabNetEncoderStep_list = [TabNetEncoderStep(input_dim, self.Sharedacrossdecisionstep, Nd, Na, gamma) for _ in range(step_num)]
    ...
    def forward(self, f, S=None):
        ...
        for i in range(self.step_num):
            a, d, mask, agg, prior_scales = self.TabNetEncoderStep_list[i](a, bn_f, prior_scales)
            total_agg += mask*agg
            embeding = embeding + d
            if total_mask != None:
                total_mask = torch.cat([total_mask.view(-1, total_mask.shape[1], total_mask.shape[2]), mask.view(-1, mask.shape[0], mask.shape[1])], dim=0)
            else:
...
class TabNetDecoder(nn.Module):
    def __init__(self, input_dim, output_dim, step_num):
        ...
        self.TabNetDecoderStep_list = [TabNetDecoderStep(input_dim, output_dim, self.Sharedacrossdecisionstep) for _ in range(self.step_num)]
    def forward(self, x):
        reconstructed_features = torch.zeros((x.shape[0], self.output_dim))
        for i in range(self.step_num):
            reconstructed_features = reconstructed_features + self.TabNetDecoderStep_list[i](x)
        return reconstructed_features

class TabNet(nn.Module):
    def __init__(self, batch_size, input_dim, output_dim, step_num, Nd=64, Na=64, gamma=1):
        super(TabNet,self).__init__()
        self.encoder = TabNetEncoder(batch_size, input_dim, output_dim, step_num, Nd, Na, gamma)
