...
class GhostBatchNorm(nn.Module):
    def __init__(self, num_features, num_splits, momentum=0.1):
        ...
        self.bn_list = [nn.BatchNorm1d(num_features = num_features, momentum = momentum) for _ in range(self.num_splits)]


    def forward(self, input):
        batch_size = input.shape[0]

        assert ((batch_size%self.num_splits) == 0) or (self.num_features == input.shape[1])
