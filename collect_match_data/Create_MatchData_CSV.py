...
def preprocessingRawData(rawMatchData, args):
    ...
    for matchId, matchData in matchIdToData.items():
        for participant in matchData["info"]["participants"]:
            ...
            key = "_".join(
                [participant["teamPosition"], str(participant["championId"])]
            )
...
def createDict(info=True, numOfSummoner=None, typeOfMatch=["s"], init=None):
    ...
    if init is not None:
        return {column: init for column in columns}
    else:
...
def divAndAddMatchData(dataDict, matchDataList, i, typeOfMatch, referGameNum):
    ...
    for matchData in matchDataList[start:end]:
        ...
        for key, value in matchData.items():
            if key not in match_columns["data"]:
                continue
            if key in sumDict:
                sumDict[key] += (
                    value / gameDuration / len(matchDataList[start:end])
                )  # 평균

    # info
    for key, value in matchDataList[0].items():
        if key not in match_columns["info"]:
            continue
        if key in sumDict:
            sumDict[key] = value
...
def makeDataset(matchIdToData, summonerIdToDict, args):
    ...
    for matchId, matchData in pbar:
        ...
        for participant in matchData["info"]["participants"]:
            teamIdToData[
                "_".join([str(participant["teamId"]), participant["teamPosition"]])
            ] = participant
        ...
        for participant in teamIdToData.values():
            if participant["summonerId"] not in summonerIdToDict:
                matchCheck = False
            if participant["teamPosition"] not in line:
                matchCheck = False
        if not matchCheck:
            continue
        ...
        for i, team in enumerate(teamAndLines):
            ...
            matchDataList = sorted(
                summonerIdToDict[summonerId][lineAndChamp],
                key=lambda x: x["gameCreation"],
                reverse=True,
            )
            start = [matchData["matchId"] for matchData in matchDataList].index(matchId)
            dataDict = divAndAddMatchData(
                dataDict, matchDataList[start:], i, "s", args.refer_num
            )

            # label data
            gameDuration = participant["gameDuration"] / 60
            for key, value in participant.items():
                if key in match_columns["data"]:
                    value /= gameDuration
                key = "_".join([key, str(i), "l"])
                if key in labelDict:
                    labelDict[key].append(value)
        ...
        pbar.set_description(
            f"Curr : {createTime:.6f}s, Mean : {sum(timeLog) / len(timeLog):.6f}s, Max : {max(timeLog):.6f}s, Min : {min(timeLog):.6f} "
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--data_path", default="./data", type=str, help="Data directory path"
    )
    parser.add_argument(
        "--raw_data",
    ...
    parser.add_argument(
        "--refer_num", default=20, type=int, help="Num of game to refer"
    )
